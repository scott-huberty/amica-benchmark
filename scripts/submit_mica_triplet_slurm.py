...
DEFAULT_SBATCH = Path(__file__).resolve().parents[1] / "slurm" / "mica_triplet_dataset.sbatch"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Submit one SLURM job per dataset; each job runs Fortran, "
            "AMICA-Python EM, and AMICA-Python DAAREM serially on one allocation."
        )
    )
    ...
    parser.add_argument("--run-tag", default="mica_release_triplet_slurm")
    ...
    parser.add_argument(
        "--daarem-accelerator-order",
        type=int,
        default=None,
        help="Override DAAREM accelerator_order. Leave unset to use AMICA-Python defaults.",
    )
    parser.add_argument(
        "--container-runtime",
    ...
    for ds in datasets:
        ...
        cmd.extend(
            [
                ...
                "" if args.daarem_accelerator_order is None else str(int(args.daarem_accelerator_order)),
            ]
        )
    ...
    print(f"Submitted {len(datasets)} triplet jobs. Batch dir: {batch_dir}")
